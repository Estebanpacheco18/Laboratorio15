...
export default function EditarEspecialidad() {
  ...
  useEffect(() => {
    const fetchEspecialidad = async () => {
      const res = await fetch(`http://localhost:3001/api/especialidades/${CodEspec}`);
      const data = await res.json();
      setDescripcionEsp(data.descripcionEsp);
    };
    fetchEspecialidad();
  }, [CodEspec]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await fetch(`http://localhost:3001/api/especialidades/${CodEspec}`, {
      method: 'PUT',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#232526]">
      <div className="w-full max-w-md bg-[#181f2b] rounded-2xl shadow-2xl p-10 border border-[#2e374a] relative">
        ...
        <h2 className="text-3xl font-extrabold text-center text-[#00c6ff] mb-8 tracking-wide drop-shadow-lg">Editar Especialidad</h2>

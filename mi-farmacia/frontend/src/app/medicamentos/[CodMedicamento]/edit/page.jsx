...
import { getMedicamento, updateMedicamento } from '@/lib/apiMedicamentos';
...
export default function EditarMedicamento() {
  ...
  useEffect(() => {
    getMedicamento(CodMedicamento).then(data => {
      setDescripcionMed(data.descripcionMed);
      setStock(data.stock);
      setPrecioVentaUni(data.precioVentaUni);
      setCodEspec(data.CodEspec);
    });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await updateMedicamento(CodMedicamento, { descripcionMed, stock, precioVentaUni, CodEspec });
    router.push('/medicamentos');
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#232526]">
      <div className="w-full max-w-lg bg-[#181f2b] rounded-2xl shadow-2xl p-10 border border-[#2e374a] relative">
        ...
        <h2 className="text-3xl font-extrabold text-center text-[#00c6ff] mb-8 tracking-wide drop-shadow-lg">Editar Medicamento</h2>
